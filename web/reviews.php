...
<script>
	...
	{
		updateIframe(getUrlVars()['cluster'], getUrlVars()['page'], 'aveStars', getUrlVars()['search']);
		document.getElementById('currPage').innerHTML=getUrlVars()['page'];
	}

	function updateIframe(cluster, page, sort, search)
	{
		document.getElementById('theIframe').src='reviewerList.php?cluster='+cluster+'&page='+page+'&sort='+sort+'&search='+search;
	}
		...
		<?php include('header2.php');?>
			<div class='row-fluid' id='profile'>
				...
				<tr>
					...
					<td class='span9'>
						<a class='btn btn-small' onclick='changeSort("aveStars");'>Highest Average Rating</a>
						<a class='btn btn-small' onclick='changeSort("reviewsGiven");'>Most Reviews</a>
						<a class='btn btn-small' onclick='changeSort("useful");'>Most Useful</a>
					</td>
			</table>
		</div>
		<div class='row-fluid'>
